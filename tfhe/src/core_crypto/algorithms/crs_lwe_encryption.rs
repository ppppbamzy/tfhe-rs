...
/// Convenience function to share the core logic of the CRSLWE encryption between all functions needing
/// it.
pub fn fill_crs_lwe_mask_and_body_for_encryption<Scalar, KeyCont, InputCont, OutputCont, Gen>(
    crs_lwe_secret_key: &CRSLweSecretKey<KeyCont>,
    output_mask: &mut CRSLweMask<OutputCont>,
    output_body: &mut CRSLweBody<OutputCont>,
    ...
    Gen: ByteRandomGenerator,
{
    ...
    //let mask_ref = output_mask.as_ref();
    ...
    let bodies_mut = output_body.as_mut();
    //let data_ref = output_body.as_ref();
    generator.fill_slice_with_random_noise_custom_mod(bodies_mut, noise_parameters, ciphertext_modulus);
    
    slice_wrapping_add_assign(bodies_mut, encoded.as_ref());
    ...
    bodies_mut.iter_mut().zip(keys).for_each(|(body, key_chunk)| *body = (*body).wrapping_add(slice_wrapping_dot_product(
        output_mask.as_ref(),
        key_chunk,
    )) );
    ...
    Gen: ByteRandomGenerator,
{
    ...
    fill_crs_lwe_mask_and_body_for_encryption(
        crs_lwe_secret_key,
        &mut mask,
        &mut body,
        encoded,
