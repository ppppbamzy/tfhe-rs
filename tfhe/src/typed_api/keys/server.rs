...
// Serde does not derives Serialize/Deserialize for `Rc` and `Arc` types
...
//
// Note: serde has a `rc` feature to allow deriving on Arc and Rc types
// but activating it in our lib would mean also activate it for all the depdency stack,
...
impl<'de> serde::Deserialize<'de> for ServerKey {
    ...
    where
        ...
        DeserializableServerKey::deserialize(deserializer)
        .map(|deserialized| {
            Self {
                #[cfg(feature = "boolean")]
